# Computation with kernels for fittig the derivative function using data-adaptive basis functions
...
def gram_traj(S, T, kernel='gauss', sigma=0.2):
    ...
    BB = map(func, SS.tolist(), TT.tolist())    # func(si,tj)
    BB = np.array(list(BB))
    Phi = BB.reshape((n,m))    # nxm array, values of n basis function for trajectory at m time points
