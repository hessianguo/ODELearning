...
X_dot_rkhs, X_fit, lamb1 = denoise_vrkhs(T, X_ns, None, 'auto', kernel_type, (0.4,))
err_rkhs = compute_l2norm(X_dot_rkhs-Dx[:,1:], T[1:])/compute_l2norm(Dx[:,1:], T[1:])

# Compute the error using TV regularization
dt = T[1] - T[0]
X_dot_tv_1 = TVRegDiff(X_ns[0,:], 50, 2e-2, scale='small', ep=1e-6, dx=dt, plotflag=0)
X_dot_tv_2 = TVRegDiff(X_ns[1,:], 50, 2e-2, scale='small', ep=1e-6, dx=dt, plotflag=0)
X_dot_tv = np.vstack((X_dot_tv_1, X_dot_tv_2))
...
filename = f"numerical_errors_lotka_{nsr:.2e}.txt"
...
filename = f"derivx_rkhs_lotka_nsr_{nsr:.2e}.pdf"
...
plt.xlabel('$t$', fontsize=10)
plt.ylabel('$\\dot{x}_1$', fontsize=10)
filename = f"derivx_fd_lotka_nsr_{nsr:.2e}.pdf"
plt.savefig(filename, format="pdf", bbox_inches="tight")
plt.show()
...
plt.xlabel('$t$', fontsize=10)
plt.ylabel('$\\dot{x}_1$', fontsize=10)
filename = f"derivx_tv_lotka_nsr_{nsr:.2e}.pdf"
plt.savefig(filename, format="pdf", bbox_inches="tight")
plt.show()
...
filename = f"noisedata_x_lotka_nsr_{nsr:.2e}.pdf"
plt.savefig(filename, format="pdf", bbox_inches="tight")
plt.show()
plt.plot(T, X_data[0], '--b', label='true x1')
...
plt.xlabel('$t$', fontsize=10)
plt.ylabel('$x_1$', fontsize=10)
filename = f"denoisedata_x_lotka_nsr_{nsr:.2e}.pdf"
plt.savefig(filename, format="pdf", bbox_inches="tight")
plt.show()